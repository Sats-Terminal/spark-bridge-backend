...
use gateway_spark_service::utils::{create_wrunes_metadata, convert_network_to_spark_network};
...
pub async fn handle(
    ...
    let response = flow_processor.storage.get_issuer_musig_id().await
    ...
    let issuer_musig_id = match response {
        ...
        None => {
            ...
            let rune_id = flow_processor.storage.get_row_by_deposit_address(request.btc_address.to_string()).await
                .map_err(|e| FlowProcessorError::DbError(e))?
            ...
            flow_processor.frost_aggregator.run_dkg_flow(&musig_id).await
                .map_err(|e| FlowProcessorError::FrostAggregatorError(format!("Failed to run DKG flow for issuer: {}", e)))?;

            let wrunes_metadata = create_wrunes_metadata(rune_id);

            flow_processor.spark_service.send_spark_transaction(
                musig_id.clone(),
                None,
                ...
                .await
                .map_err(|e| FlowProcessorError::SparkServiceError(format!("Failed to send spark create transaction: {}", e)))?;
    ...
    let deposit_addr_info = flow_processor.storage
        .get_row_by_deposit_address(request.btc_address.to_string()).await
        .map_err(|e| FlowProcessorError::DbError(e))?
        .ok_or(FlowProcessorError::InvalidDataError("Deposit address info not found".to_string()))?;

    let receiver_identity_public_key = bitcoin::secp256k1::PublicKey::from_str(&deposit_addr_info.bridge_address.ok_or(FlowProcessorError::InvalidDataError("Bridge address not found".to_string()))?)
        .map_err(|e| FlowProcessorError::InvalidDataError(format!("Failed to parse receiver identity public key: {}", e)))?;
