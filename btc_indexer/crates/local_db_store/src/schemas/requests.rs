...
#[async_trait]
pub trait RequestsStorage: Send + Sync {
    async fn get_watch_request(&self, id: Uuid) -> Result<Option<WatchRequest>, DbError>;
    async fn get_all_unprocessed_watch_requests(&self) -> Result<Vec<WatchRequest>, DbError>;
    async fn insert_watch_request(&self, watch_request: WatchRequest) -> Result<(), DbError>;
    async fn update_watch_request_status(&self, id: Uuid, status: ValidationResult) -> Result<(), DbError>;
}

#[async_trait]
impl RequestsStorage for LocalDbStorage {
    async fn get_watch_request(&self, id: Uuid) -> Result<Option<WatchRequest>, DbError> {
    ...
    async fn insert_watch_request(&self, watch_request: WatchRequest) -> Result<(), DbError> {
        ...
        sqlx::query(
            "INSERT INTO btc_indexer.requests (id, request_id, outpoint, btc_address, rune_id, rune_amount, sats_amount, created_at, status, error_details, callback_url)
