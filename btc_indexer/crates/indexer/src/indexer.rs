...
impl<Api: BtcIndexer> Indexer<Api> {
            ...
            if self.local_db_store.exists(watch_request.outpoint.txid).await? {
                ...
                match validation_result.watch_request_status {
                    WatchRequestStatus::Confirmed | WatchRequestStatus::Failed => {
                        self.local_db_store
                            .update_watch_request_status(watch_request.id, validation_result.clone())
                            .await?;
                ...
                let (validation_result, validation_metadata) =
                    self.validate_transaction_timeout(watch_request.clone()).await?;
                match validation_result.watch_request_status {
                    WatchRequestStatus::Confirmed => {
                        tracing::error!("Invalid watch request status confirmed");
                        return Err(IndexerError::InvalidData(
                            "Invalid watch request status confirmed".to_string(),
                        ));
                    }
    ...
    async fn validate_transaction_timeout(
        &self,
        watch_request: WatchRequest,
    ) -> Result<(ValidationResult, Option<ValidationMetadata>), IndexerError> {
        let cur_timestamp = Utc::now();
        if Duration::from_millis(
            (cur_timestamp.timestamp_millis() as u64)
                .saturating_sub(watch_request.created_at.timestamp_millis() as u64),
        ) > Duration::from_millis(self.config.validation_timeout_millis)
        {
            tracing::error!(
                "Timeout waiting for transaction output for outpoint: {}",
                watch_request.outpoint
            );
            return Ok((
                ValidationResult {
                    watch_request_status: WatchRequestStatus::Failed,
                    error_details: Some(WatchRequestErrorDetails::Timeout(format!(
                        "Timeout waiting for transaction output for outpoint: {}",
                        watch_request.outpoint
                    ))),
                },
    ...
    async fn validate_deposit_transaction(
        &self,
        watch_request: WatchRequest,
        outpoint_data: OutPointData,
        blockchain_info: BlockchainInfo,
    ) -> Result<(ValidationResult, Option<ValidationMetadata>), IndexerError> {
        if blockchain_info.block_height.saturating_sub(outpoint_data.block_height)
            < self.config.confirmation_block_height_delta
        {
            tracing::debug!(
                "Blockchain height is not confirmed yet for outpoint: {}",
                watch_request.outpoint
            );
            return Ok((
                ValidationResult {
        ...
        if let Some(rune_id) = watch_request.rune_id {
            ...
            if rune_amount != expected_rune_amount {
                tracing::error!(
                    "Invalid rune amount: expected: {}, got: {}",
                ...
                return Ok((
                    ValidationResult {
                        watch_request_status: WatchRequestStatus::Failed,
                        error_details: Some(WatchRequestErrorDetails::InvalidRuneAmount(format!(
                            "Invalid rune amount: expected: {}, got: {}",
        ...
        if let Some(expected_sats_amount) = watch_request.sats_amount {
            ...
            if sats_amount != expected_sats_amount {
                tracing::error!(
                    "Invalid sats amount: expected: {}, got: {}",
                ...
                return Ok((
                    ValidationResult {
                        watch_request_status: WatchRequestStatus::Failed,
                        error_details: Some(WatchRequestErrorDetails::InvalidSatsAmount(format!(
                            "Invalid sats amount: expected: {}, got: {}",
        ...
        Ok((
            ValidationResult {
                watch_request_status: WatchRequestStatus::Confirmed,
                error_details: None,
            },
    ...
    async fn send_notify_request(
        ...
        validation_metadata: Option<ValidationMetadata>,
    ) -> Result<(), IndexerError> {
        match validation_result.watch_request_status {
            WatchRequestStatus::Confirmed => {
                let validation_metadata = validation_metadata
                    .ok_or(IndexerError::InvalidData("Validation metadata not found".to_string()))?;
                tracing::info!(
                    "Sending notify request for confirmed watch request for outpoint: {}",
                    watch_request.outpoint
                );
