...
impl DepositAddressStorage for LocalDbStorage {
    ...
    async fn set_txid(&self, address: String, txid: Txid) -> Result<(), DbError> {
        let _ = sqlx::query("UPDATE gateway.deposit_address SET txid = $1 WHERE address = $2")
            .bind(txid.to_string())
            .bind(address)
            .execute(&self.get_conn().await?)
            .await
    ...
    async fn update_confirmation_status_by_txid(
        ...
        let _ = sqlx::query("UPDATE gateway.deposit_address SET confirmation_status = $1 WHERE txid = $2")
            .bind(Json(confirmation_status))
            .bind(txid.to_string())
            .execute(&self.get_conn().await?)
            .await
        ...
        match result {
            Some((address,)) => {
                let address = Address::from_str(&address)
                    .map_err(|e| DbError::DecodeError(format!("Failed to decode address: {}", e)))?;
