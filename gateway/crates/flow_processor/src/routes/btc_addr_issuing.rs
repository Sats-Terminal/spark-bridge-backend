...
pub async fn handle(
    ...
    let (public_key_package, user_uuid, rune_id, dkg_share_id) =
        match local_db_storage.get_ids_by_musig_id(&request.musig_id).await? {
            None => {
                ...
                match local_db_storage.get_dkg_share_agg_data(&dkg_share_id).await? {
                    ...
                    Some(AggregatorDkgShareData { dkg_state }) => {
                        ...
                        match dkg_state {
                            ...
                            } => (pubkey_package, user_uuid, rune_id, dkg_share_id),
                        }
                    }
                }
            }
