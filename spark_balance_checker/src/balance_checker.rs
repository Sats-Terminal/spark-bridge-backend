...
use crate::utils::decode_spark_address::decode_spark_address;
use eyre::Result;
use hex;
use log;
...
impl SparkBalanceChecker {
    ...
    pub(crate) async fn get_client(&mut self) -> Result<SparkServiceClient<Channel>, ServerError> {
    ...
    pub(crate) async fn retry_query<F, Fut, Resp, P>(
        &mut self,
        query_fn: F,
        params: P,
        ...
        P: Clone,
    {
        ...
        Err(ServerError::ConnectionError(
            "All retry attempts failed".to_string(),
        ))
    }

    pub(crate) async fn get_token_outputs(
        &mut self,
        ...
        let identity_public_key = hex::decode(address_data.identity_public_key).map_err(|e| {
            ServerError::DecodeError(format!("Failed to decode identity public key: {}", e))
        })?;
        let token_identifier = bech32::decode(&token_identifier)
            .map_err(|e| {
                ServerError::DecodeError(format!("Failed to decode token identifier: {}", e))
            })?
            .1;

        let query_fn = |mut client: SparkServiceClient<Channel>,
                        params: (Vec<u8>, Vec<u8>, Network)| async move {
            ...
            client.query_token_outputs(request).await.map_err(|e| {
                ServerError::ConnectionError(format!("Failed to query balance: {}", e))
            })
        };

        self.retry_query(
            query_fn,
            (identity_public_key, token_identifier, address_data.network),
        )
        .await
        .map(|r| r.into_inner())
    }
...
mod tests {
    ...
    async fn test_get_balances_direct() {
        ...
        let address =
            "sprt1pgss8fxt9jxuv4dgjwrg539s6u06ueausq076xvfej7wdah0htvjlxunt9fa4n".to_string();
        let rune_id =
            "btknrt1p2sy7a8cx5pqfm3u4p2qfqa475fgwj3eg5d03hhk47t66605zf6qg52vj2".to_string();

        let config = Config::new(None).unwrap();

        let mut balance_checker = SparkBalanceChecker::new(config.spark);

        let response = balance_checker
            .get_token_outputs(address, rune_id)
            .await
            .unwrap();
        ...
        for output in response.outputs_with_previous_transaction_data {
            if let Some(output) = output.output {
                log::info!(
                    "token identifier: {:?}",
                    hex::encode(output.token_identifier.unwrap())
                );
                log::info!(
                    "token pubkey: {:?}",
                    hex::encode(output.token_public_key.unwrap())
                );
