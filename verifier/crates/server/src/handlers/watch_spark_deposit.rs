...
pub async fn handle(
    ...
    let response = state.spark_balance_checker_client.get_balance(GetBalanceRequest {
        spark_address: request.address.to_string(),
        rune_id: request.musig_id.get_rune_id(),
    }).await.map_err(|e| VerifierError::SparkBalanceCheckerClientError(format!("Failed to get balance: {}", e)))?;
