...
impl AggregatorSignSessionStorage for Storage {
    async fn get_sign_data(&self, musig_id: MusigId, session_id: Uuid) -> Result<Option<AggregatorSignData>, DatabaseError> {
        ...
        let result: Option<(Json<AggregatorSignState>, Json<SigningMetadata>, Vec<u8>, Option<Vec<u8>>)> = sqlx::query_as(
        ...
        Ok(result.map(|(json_sign_state, json_metadata, message_hash, tweak)| AggregatorSignData {
    ...
    async fn set_sign_data(&self, musig_id: MusigId, session_id: Uuid, sign_data: AggregatorSignData) -> Result<(), DatabaseError> {
