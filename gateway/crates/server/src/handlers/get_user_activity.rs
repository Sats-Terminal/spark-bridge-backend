...
fn overall_status(info: &DepositAddrInfo, utxo_status: Option<UtxoStatus>) -> String {
    if info
        ...
        .responses
        .values()
        .any(|status| matches!(status, DepositStatus::Failed(_)))
    {
    ...
    match utxo_status {
        ...
        _ => {
            if info.confirmation_status.check_all_verifiers_confirmed() {
                ...
                .responses
                .values()
                .any(|status| matches!(status, DepositStatus::WaitingForConfirmation))
            {
                ...
                .responses
                .values()
                .all(|status| matches!(status, DepositStatus::Created))
            {
...
async fn build_activity_response(
    ...
    for record in activity.into_iter() {
        ...
        let rune_id = info.musig_id.get_rune_id();
        ...
        let wrune_metadata = metadata_map.get(&rune_id).cloned();
