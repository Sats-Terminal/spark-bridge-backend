...
impl Signer {
    pub fn new(id: u16, max_signers: u16, min_signers: u16) -> Self {
        Self {
            rng: OsRng,
            max_signers,
            min_signers,
    ...
    pub fn dkg_part1(&mut self) -> Result<Round1Package> {
        let (secret_package, package) = part1(self.identifier, self.max_signers, self.min_signers, self.rng)?;
    ...
    ) -> Result<BTreeMap<Identifier, Round2Package>> {
        let round1_secret_package = self
            .round1_secret_package
            .clone()
            .ok_or(eyre!("Secret package not found"))?;
        let (secret_package, package) = part2(round1_secret_package, round1_packages)?;
    ...
    pub fn dkg_part3(&mut self, round2_packages: &BTreeMap<Identifier, Round2Package>) -> Result<PublicKeyPackage> {
        let round2_secret_package = self
            ...
            .clone()
            .ok_or(eyre!("Secret package not found"))?;
        let round1_packages = self.round1_packages.clone().ok_or(eyre!("Round1 packages not found"))?;
    ...
    pub fn sign_part1(&mut self) -> Result<SigningCommitments> {
        ...
        let (nonce, commitment) = commit(key_package.signing_share(), &mut self.rng);
...
impl Aggregator {
    ...
    pub fn aggregate_part1(
        &mut self,
        signing_commitments: BTreeMap<Identifier, SigningCommitments>,
    ) -> Result<SigningPackage> {
        let signing_package = frost_secp256k1_tr::SigningPackage::new(signing_commitments, &self.message);
    ...
    pub fn aggregate_part2(&mut self, signature_shares: BTreeMap<Identifier, SignatureShare>) -> Result<Signature> {
        ...
        let group_signature =
            frost_secp256k1_tr::aggregate(&signing_package, &signature_shares, &self.public_key_package)?;
