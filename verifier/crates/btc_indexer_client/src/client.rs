...
impl BtcIndexerClient {
    ...
    pub async fn watch_runes_deposit(&self, request: WatchRunesDepositRequest) -> Result<(), BtcIndexerClientError> {
        let url = self.config.address.join(WATCH_RUNES_DEPOSIT_PATH)
        ...
        if response.status().is_success() {
            ...
            Err(BtcIndexerClientError::HttpError(format!("Failed to send HTTP request with status {}, error: {}", response.status(), response.text().await.unwrap_or_default())))
        }
    }
}
